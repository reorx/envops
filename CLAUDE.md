...
- Run the CLI directly: `./envops show <file>` (shebang runs it via uv)
...
3. **Fix** `looks_like_secret` / the regexes in `envops`, then get all tests green.
...
5. **Verify end-to-end** on the real .env file(s) with `./envops show` — tests use synthetic data; the report came from a real file.
...
- `scripts/diagnose_false_positives.py <env-file>` — for every masked key, prints which rules matched (`value-prefix`, `url-password`, `key-name`, entropy with score/length). Prints key names and rule metadata only, never values.
...
- `envops` is a single file without a `.py` extension, so scripts can't import it; they load it with the exec pattern (read source, split at `if __name__ == '__main__':`, `exec` into a module namespace). Reuse that pattern.
...
- Inspect files under investigation with `./envops show` (masked), not `cat`.
...
1. Positive value checks run first and are never exempted: known credential prefixes (`SECRET_VALUE_RE`), URL with embedded password (`URL_WITH_PASSWORD_RE`). This is why `DATABASE_URL=postgres://u:p@h/db` stays masked even though the key ends in `URL`.
